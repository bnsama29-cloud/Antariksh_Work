...
add_para(doc, "Synthetic images generated by the pipeline simulating the Raspberry Pi camera payload. As the fungi grow, the optical density (OD600) of the fluid increases, visibly darkening the chambers over the 48-hour mission.")

# Add images in a 3-column table
cam_table = doc.add_table(rows=2, cols=3)
cam_table.style = 'Table Grid'
hdr_cells = cam_table.rows[0].cells
hdr_cells[0].text = "T = 0 hours (Inoculation)"
hdr_cells[1].text = "T = 24 hours (Mid-orbit)"
hdr_cells[2].text = "T = 48 hours (Completion)"

img_cells = cam_table.rows[1].cells
p0 = img_cells[0].paragraphs[0]
r0 = p0.add_run()
r0.add_picture(str(BASE_DIR / "figures/camera/img_00h.png"), width=Inches(2.0))

p1 = img_cells[1].paragraphs[0]
r1 = p1.add_run()
r1.add_picture(str(BASE_DIR / "figures/camera/img_24h.png"), width=Inches(2.0))

p2 = img_cells[2].paragraphs[0]
r2 = p2.add_run()
r2.add_picture(str(BASE_DIR / "figures/camera/img_48h.png"), width=Inches(2.0))
